...
class ProxyMiddleWare(HttpProxyMiddleware):
    def process_request(self, request, spider):
        ...
        # random_proxy_index=1
        proxy = proxy_list[random_proxy_index]
        proxys = proxy.split("@")
        print "https://{0}".format(proxys[-1])
        request.meta["proxy"] = "https://{0}".format(proxys[-1])
        proxy_user_pass = proxys[0]
        user_pass = to_bytes(proxy_user_pass)
        encoded_user_pass = base64.b64encode(user_pass).strip()
...
class CustomHeadersMiddleware(DefaultHeadersMiddleware):
    ...
    def process_request(self, request, spider):
        if request.headers:
            request.headers["user-agent"] = self.user_agent[random.randint(0, len(self.user_agent))]
        pass
...
class CustomCookiesMiddleware(CookiesMiddleware):
    def getRandomId(self):
        ...
        r = int(time.time() * 1000 / 6e4)
        e = ""
        for i in range(5):
            e += t[int(random.random() * (len(t) - 1))]
        return e + str(r)

    def process_request(self, request, spider):
        request.cookies = {
            "kr_stat_uuid": self.getRandomId()
        }
